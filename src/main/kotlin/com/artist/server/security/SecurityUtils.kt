...
fun getAuthenticationOrNull() = SecurityContextHolder.getContext().authentication ?: null
...
fun getCurrentUserLogin(): Optional<String> =
    Optional.ofNullable(extractPrincipal(getAuthenticationOrNull()))
...
fun getCurrentUserJWT(): Optional<String> =
    Optional.ofNullable(getAuthenticationOrNull())
...
fun isAuthenticated(): Boolean {
    ...
    if (authentication != null) {
        val isAnonymousUser = getAuthorities(authentication)?.none { it == StdAuthority.CODE_AUTHORITY_ANONYMOUS }
